...
if [ -f "$ENV_FILE" ]; then
  ...
  OPS_FILL_DOWNLOAD_DIR=$(grep -E '^OPS_FILL_DOWNLOAD_DIR=' "$ENV_FILE" | head -1 | sed 's/^OPS_FILL_DOWNLOAD_DIR=//' | tr -d '"'"'"' ')
fi
...
# 若 .env 未配置 OPS_FILL_DOWNLOAD_DIR，自动基于 DOWNLOAD_DIR 派生 <DOWNLOAD_DIR>/ops-fill
if [ -z "$OPS_FILL_DOWNLOAD_DIR" ] && [ -n "$DOWNLOAD_DIR" ] && [ -f "$ENV_FILE" ]; then
  OPS_FILL_DOWNLOAD_DIR="${DOWNLOAD_DIR%/}/ops-fill"
  {
    echo ""
    echo "# OPS MP3 补全专用种子下载目录（由 start.sh 自动生成）"
    echo "OPS_FILL_DOWNLOAD_DIR=${OPS_FILL_DOWNLOAD_DIR}"
  } >> "$ENV_FILE"
  info ".env 已追加 OPS_FILL_DOWNLOAD_DIR=${OPS_FILL_DOWNLOAD_DIR}"
fi

if [ -n "$OPS_FILL_DOWNLOAD_DIR" ]; then
  mkdir -p "$OPS_FILL_DOWNLOAD_DIR"
  info "OPS 补全目录已就绪：${OPS_FILL_DOWNLOAD_DIR}"
else
  warn "未在 .env 中找到 OPS_FILL_DOWNLOAD_DIR 且 DOWNLOAD_DIR 缺失，跳过创建 OPS 补全目录（若不需要 OPS 补全功能可忽略）。"
fi
...
echo ""
echo "  2. 启动 Worker（PM2 后台运行，推荐）："
echo "     ${YELLOW}pm2 delete music-upload-worker 2>/dev/null; pkill -f '[m]usic-worker' 2>/dev/null; fuser -k 36501/tcp 2>/dev/null${RESET}"
echo "     ${YELLOW}cd ${TARGET_DIR} && pm2 start music-worker.js --name music-upload-worker && pm2 save${RESET}"
echo "     ${YELLOW}pm2 logs music-upload-worker${RESET}  # 查看日志，正常后 Ctrl+C 退出"
echo ""
echo "  或直接前台运行（调试用）："
